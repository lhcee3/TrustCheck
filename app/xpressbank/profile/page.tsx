...
import {
  ArrowLeft, ShieldCheck, BadgeCheck, LogOut, Bell, Copy,
  CheckCircle, CreditCard, Loader2, Zap, Receipt, Users,
} from "lucide-react";
      ...
      <div className="hidden sm:flex flex-col flex-1">
        <header className="bg-white border-b border-stone-100 px-8 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-8 h-8 rounded-full bg-[#2D6A4F] flex items-center justify-center text-white text-xs font-bold">{user?.avatarInitials ?? "RK"}</div>
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-[#2D6A4F] rounded-2xl p-6 flex flex-col items-center gap-3 text-center">
                <div className="w-20 h-20 rounded-full bg-white/20 flex items-center justify-center text-white text-3xl font-bold">{user?.avatarInitials ?? "RK"}</div>
                <div>
                ...
                <div className="flex items-center justify-between px-4 py-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">{notifStatus === "granted" ? "✅ Active" : notifStatus === "denied" ? "❌ Blocked" : "Get fraud alerts"}</p>
                    </div>
                  </div>
              ...
              <button onClick={logout} className="w-full rounded-2xl border border-red-200 bg-red-50 py-3.5 text-sm font-bold text-red-600 flex items-center justify-center gap-2 hover:bg-red-100 transition-colors">
                <LogOut className="w-4 h-4" /> Logout
              </button>
            </div>

            {/* Right column — personal info + linked cards */}
            <div className="col-span-2 flex flex-col gap-4">
              <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-stone-50"><h2 className="text-sm font-bold text-slate-800">Personal Information</h2></div>
                {infoRows.map((row) => (
              ...
              {user?.linkedCards?.map((card) => (
                <div key={card.last4} className="bg-white rounded-2xl border border-stone-100 shadow-sm p-5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-[#2D6A4F]/10 flex items-center justify-center"><CreditCard className="w-6 h-6 text-[#2D6A4F]" /></div>
                    <div>
                  ...
                  <span className="text-xs text-emerald-600 font-medium bg-emerald-50 border border-emerald-100 px-3 py-1 rounded-full">Active</span>
                </div>
              ))}
            </div>
          </div>
