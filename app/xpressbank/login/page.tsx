...
export default function XpressBankLogin() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem("xb_user", JSON.stringify(res.user));
  ...
  const form = (
    <form onSubmit={handleLogin} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        ...
        <input
          type="text" placeholder="test@gmail.com" value={email} required
  ...
  return (
    <>
      ...
      <div className="hidden sm:flex min-h-screen bg-[#F7F6F2] items-center justify-center">
        <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl border border-stone-100 overflow-hidden">
          <div className="bg-[#2D6A4F] px-8 py-8 flex flex-col items-center gap-3 text-center">
            <div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center">
              <Zap className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-white">XpressBank</h1>
              <p className="text-emerald-200 text-xs mt-1">Secure · Fast · Protected</p>
            </div>
          </div>
          <div className="px-8 py-8 flex flex-col gap-5">
            ...
            <p className="text-center text-xs text-slate-400">Demo: test@gmail.com / Password</p>
          </div>
        </div>
      </div>

      {/* ── Mobile: original layout unchanged ── */}
      <div className="sm:hidden min-h-screen bg-[#2D6A4F] flex flex-col">
